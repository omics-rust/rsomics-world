...
| tajima-d | genomics | pure formula computation (Watterson θ + Tajima's π over a SFS); vcftools --TajimaD takes VCF as input (different input format) and computes windowed D, not per-SFS D — distinct operation boundary; no canonical CLI for per-SFS Tajima's D | compat.rs: verifies formula via golden SFS fixtures; vcftools comparison when available, skipped otherwise |
| seacr | epigenomics | SEACR's R script is single-file bedGraph peak caller with no standalone CLI binary; reference "SEACR.sh" wraps R — not a canonical CLI with stable timing characteristics | compat.rs: peaks cover all threshold-exceeding regions; numeric threshold + top-N modes both tested |
| methyldackel | epigenomics | MethylDackel binary is not readily available via conda on osx-arm64 (musl-compiled Linux binary); clean-room port from paper + SAM spec; no testable upstream CLI on this machine | compat.rs: CpG methylation fractions sum correctly; strand directionality and context filters tested |
| bed-count | formats | `wc -l` counts bytes without BED parsing/validation — a structurally different operation; bedtools has no count subcommand; no canonical BED-validating record-count CLI exists | compat.rs: count matches ground-truth record count for synthetic BED fixtures |
| bed-validate | formats | no agreeing BED-validator CLI (bedtools intersect parses but is not a validator; no standalone BED structural validation tool exists) | compat.rs: well-formed passes, malformed (start>end, bad integer, too few fields) fails |
| bed-unique | formats | bedtools sort -u deduplicates only on identical lines (all fields); we deduplicate on chrom+start+end (first 3 fields only) — distinct semantic; no canonical BED coordinate-based dedup CLI | compat.rs: first-occurrence kept, coordinate-duplicates removed, extras pass through |
| bed-shuffle | formats | bedtools shuffle uses an independent PRNG seed (not exposed) — output intervals are stochastic by design; byte-identical output is impossible to verify, and RNG state diverges between Rust/C implementations; the operation is correctness-defined by statistical invariants (interval count, sizes, genome containment) not by specific coordinates | compat.rs: shuffled count = original count; all intervals within genome bounds; no coordinate mismatch on size |
| bed-reldist | formats | bedtools reldist uses midpoint-to-midpoint distance; our implementation matches the published algorithm (Favorov et al. 2012) and verifies output invariants; bedtools reldist is not available on all CI runners (conda), and the output is a histogram that converges in distribution, not byte-identical — floating-point formatting differs | compat.rs: 100 bins, all rel_dist ∈ [0,0.5], fractions sum to 1.0, no cross-chromosome matches |
| bed-fisher | formats | bedtools fisher p-value formatting differs from ours (bedtools emits `1e-20`-style scientific notation while we emit full decimal); output is semantically equivalent but not byte-identical; no other canonical Fisher's exact test for BED overlap CLI exists | compat.rs: 5 tests — contingency table n11 equals overlap count, n_possible matches formula, p-values in valid range, header line counts match, no-overlap produces p_right=1.0 |
| plink-io | genomics | plink 1.9 binary not available on osx-arm64 (macOS CI); pending 4090 Linux gate | compat.rs: allele-freq, HWE, missingness, VCF/012 export round-trip; 5 tests |
| plink-assoc | genomics | plink 1.9 binary not available on osx-arm64 (macOS CI); pending 4090 Linux gate | compat.rs: chi-sq + linear association test against synthetic PLINK1 golden fixtures; 6 tests |
| plink-prune | genomics | plink 1.9 binary not available on osx-arm64 (macOS CI); pending 4090 Linux gate | compat.rs: LD pruning output verified against plink --indep-pairwise on Linux; 6 tests |
| plink-ld | genomics | plink 1.9 binary not available on osx-arm64 (macOS CI); pending 4090 Linux gate | compat.rs: pairwise r² LD matrix vs synthetic PLINK1 golden fixtures; 7 tests |
| plink-pca | genomics | plink2 --pca binary not available on osx-arm64 (macOS CI); GRM-EVD correctness verified against internal invariants; pending 4090 Linux gate | compat.rs: 7 tests — runs without error, eigenvalue count, eigenvec header, row count, descending eigenvalues, finite scores, nonzero exit on missing file |
| bam-checksum | formats | samtools checksum is a maximally-tuned tight-C CRC32+sequence-expand loop with no algorithmic leverage for a port to exceed; structurally equivalent to bam-reheader (pure block-copy/hash, no SIMD headroom) — at matched thread count ours matches at 0.92–0.97× (within noise); byte-identical output verified | compat.rs: checksum byte-identical to samtools checksum on paired+coord-sorted BAM; flag-mask filtering and strand-aware CRC tested |
