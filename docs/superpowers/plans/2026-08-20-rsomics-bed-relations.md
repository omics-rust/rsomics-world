...
- [x] **Step 1: Add failing parser and writer tests**
...
- [x] **Step 2: Verify the new tests fail for missing accessors**

Run: `cargo test --locked bed::tests`

Expected: compilation fails because the new field accessors and writers do not exist.

- [x] **Step 3: Extend the owned record without eagerly validating optional fields**
...
- [x] **Step 4: Run focused and existing record tests**

Run: `cargo test --locked bed::tests`

Expected: all parser, optional-field, and byte-preservation tests pass.

- [x] **Step 5: Commit the record contract**
...
- [x] **Step 1: Freeze exact oracle cases before implementing**
...
- [x] **Step 2: Add failing library and CLI tests**
...
- [x] **Step 3: Verify cluster tests fail because the module and command are absent**
...
- [x] **Step 4: Implement unstranded streaming and chromosome-buffered same-strand execution**
...
- [x] **Step 5: Wire the typed CLI through shared parsing and transactional output**
...
- [x] **Step 6: Run the cluster contract and the full existing suite**
...
- [x] **Step 7: Commit, push, and wait for exact-head CI**
...
- [x] **Step 1: Add core tests around duplicate IDs and the backend boundary**
...
- [x] **Step 2: Verify the tests fail because `IntervalIndex` does not exist**

Run: `cargo test --locked interval_index`

Expected: compilation fails on the missing module and types.

- [x] **Step 3: Extract the existing COITrees machinery without changing algorithms**
...
- [x] **Step 4: Prove the refactor is behavior-neutral**
...
- [x] **Step 5: Run strict local verification and commit**
...
- [x] **Step 1: Add failing relation-index tests**
...
- [x] **Step 2: Verify the new module tests fail**

Run: `cargo test --locked relation_index`

Expected: compilation fails because `RelationBed` is absent.

- [x] **Step 3: Implement one relation wrapper over the coordinate core**
...
- [x] **Step 4: Run relation, index, and existing operation tests**
...
- [x] **Step 5: Commit the private shared relation mechanism**
...
- [x] **Step 1: Freeze pair, asymmetric, strand-relative, and report-mode oracles**
...
- [x] **Step 2: Add failing option and output tests**
...
- [x] **Step 3: Verify window tests fail because the command is absent**

Run: `cargo test --locked window`

Expected: compilation or command-tree failure naming the missing window API.

- [x] **Step 4: Implement indexed window queries and report reducers**
...
- [x] **Step 5: Wire canonical typed CLI values and conflict-checked aliases**
...
- [x] **Step 6: Run full strict verification and live oracle comparisons**
...
- [x] **Step 1: Freeze a closest semantic matrix from BEDTools**
...
- [x] **Step 2: Add failing distance and eligibility unit tests**
...
- [x] **Step 3: Verify focused tests fail on the absent engine**
...
- [x] **Step 4: Implement checked distance and eligibility types**

Use `i128` only for signed display and comparison; preserve coordinates as `u64`. Prevalidate every required B name or strand before streaming A, and validate required A fields per record. First collect eligible overlaps; when none remain, advance the start/end directional iterators only until the minimum eligible unsigned distance is known on both sides. Preserve B-file IDs for tie handling.

- [x] **Step 5: Pass the semantic engine tests**
...
- [x] **Step 6: Commit the closest engine**
...
- [x] **Step 1: Add failing output-order, placeholder, and CLI tests**

Test default B-file tie order, signed-mode directional order, first/last selection after signed ordering, no-hit and empty-B rows, optional distance `-1`, `-d`/`-D` aliases, `-t`, `-s/-S`, `-N`, stdin A, alias rejection, and incompatible selector combinations.

- [x] **Step 2: Verify tests fail before dispatch exists**
...
- [x] **Step 3: Implement tie ordering and no-hit serialization**
...
- [x] **Step 4: Wire CLI selectors through one typed `ClosestOptions` conversion**

Parse every canonical long value and each unambiguous compatibility alias into one option value before opening A or B. Support `-s`, `-S`, `-N`, `-d`, `-D`, and `-t`. Keep upstream's multi-character single-dash `-io` spelling out of the Clap interface; `--ignore-overlaps` is canonical. Reject selector conflicts through Clap; do not accept multi-database, `k`, upstream/downstream preference, or BAM flags.

- [x] **Step 5: Run strict full-suite verification**
...
- [x] **Step 6: Commit, push, and wait for exact-head CI**
...
- [x] **Step 1: Add a failing deterministic generator test**
...
- [x] **Step 2: Add `rand_core` and `rand_chacha` only as dev dependencies**
...
- [x] **Step 3: Implement 100 fixed cases per supported option family**
...
- [x] **Step 4: Run debug and release differential lanes**
...
- [x] **Step 5: Commit differential evidence**
...
- [x] **Step 1: Extend benchmark smoke fixtures and byte checks**
...
- [x] **Step 2: Run benchmark smoke and inspect scaling**
...
- [x] **Step 3: Refresh README and migration provenance**
...
- [x] **Step 4: Run the formal external-disk benchmark**
...
- [x] **Step 5: Re-run all published-operation performance checks**
