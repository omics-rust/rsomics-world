...
|---|---|---|---|
| `rsomics-common` | `1cfaa3a14c11` | supplies the demonstrated error, exit-code, JSON-envelope, runner, transaction, alias, worker, and paired-output contracts; `bed`, `seq`, and `methyl` provide concrete `write_output` call sites, and 0.12.1 batches stream and named writes in a 1 MiB buffer while propagating final flush failures | 0.12.1 published; 46 local tests, strict Clippy, rustdoc, and package verification pass; exact-head four-native-target CI `30768905163`; publish run `30768954093`; downloaded archive checksum `f9c7c567a1128f5c25084d0c65a21d237eadfe2bad941b48c11e213ab96625ab` |
...
| `rsomics-bamio` | `30459c78951f` | retains validated records and indexed SAM/BAM/CRAM input; owns checked ordinary and `CG:B,I` long-CIGAR decoding; rejects zero-sized BAM records rather than treating corruption as EOF; and loads the samtools-default appended BAI for BGZF SAM with a real region-query test | 0.8.4 published; exact-head four-native-target CI `30776588046`; publish run `30776845071`; downloaded archive checksum `22c629df18b27af31c4e6c79f318bd96d240e8e4b6c64aa6f6735354db6b7855`; BAM and methyl consumer suites green |
| `rsomics-pileup` | `8d85ae1b0cc4` | retains checked projection, retry-safe borrowed columns, per-source overlap and depth state, HTSlib-compatible BAQ, and bcftools-compatible column preparation; 0.9 adds policy-free typed consumer state retained once per active record, with `call` caching CIGAR annotation metrics and hidden BAM consensus retaining local mismatch, homopolymer, and adjusted-quality context | 0.9.0 published; exact-head four-native-target CI `31469899244`, including the samtools 1.24 differential; publish run `31471203805`; downloaded archive checksum `beae57811926a88c28ee0de8923dc7edb8d87ffe9c0afba9b47bd771a279f39e`; registry archive tests and the no-regression 250x performance gate pass |
...
7. `rsomics-bamio` previously treated a BAM record block size of zero as clean
   stream exhaustion even when it appeared before the BGZF EOF. Revision
   `08351b6` rejects the malformed record in both owned and borrowed readers.
...
The 0.6 alignment revision moves ordinary and `CG:B,I` long-CIGAR validation
from pileup into `rsomics-bamio`. Pileup revision `6005a55cface` deletes its
duplicate decoder, and methyl revision `8dac753e1d1c` tests the same API through
per-read calling. The release heads and registry evidence are recorded in the
table above; neither consumer's projection or methylation policy entered
`rsomics-bamio`.
