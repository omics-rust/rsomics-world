...
fn ours() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_rsomics-fastq-utils"))
}
...
fn head_matches_seqkit() {
    ...
    assert_eq!(our_names.len(), their_names.len(), "head count mismatch");
...
fn revcomp_matches_seqkit() {
    ...
    let our_seqs: Vec<&str> = ours
        .lines()
        .enumerate()
        .filter(|(i, _)| i % 4 == 1)
        .map(|(_, l)| l)
        .collect();
    let their_seqs: Vec<&str> = theirs
        .lines()
        .enumerate()
        .filter(|(i, _)| i % 4 == 1)
        .map(|(_, l)| l)
        .collect();
    assert_eq!(our_seqs, their_seqs, "revcomp sequences differ");
}
