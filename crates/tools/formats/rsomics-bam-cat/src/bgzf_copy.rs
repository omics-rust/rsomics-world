...
pub struct HeaderReader {
    ...
    decoded: Vec<u8>,
    /// Every uncompressed byte handed out so far — the raw BAM header bytes once
    /// the caller has drained exactly the header. Lets cat re-emit the first
    /// file's header verbatim.
    header_bytes: Vec<u8>,
}
...
impl HeaderReader {
    ...
    /// The raw uncompressed bytes consumed through this reader: exactly the BAM
    /// header (`magic l_text text n_ref refs`) once the caller has read it.
    pub fn into_consumed_header(self) -> Vec<u8> {
        self.header_bytes
    }
}
...
impl<R: Read> Read for HeaderStream<'_, R> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        self.state.header_bytes.extend_from_slice(&avail[..n]);
