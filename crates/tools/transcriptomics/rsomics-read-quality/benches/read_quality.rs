...
fn bench_ours(c: &mut Criterion) {
    ...
    let binary = format!("{target_dir}/release/rsomics-read-quality");
    ...
    c.bench_function("rsomics-read-quality", |b| {
        b.iter(|| {
            let prefix = tmp.path().join("out");
            let status = Command::new(&binary)
                .args(["-i", &bam, "-o", prefix.to_str().unwrap()])
                .status()
                .expect("failed to run rsomics-read-quality");
...
fn bench_rseqc(c: &mut Criterion) {
    ...
    c.bench_function("rseqc-read_quality.py", |b| {
        b.iter(|| {
            let prefix = tmp.path().join("ref");
            let output = Command::new(&oracle)
                .args(["-i", &bam, "-o", prefix.to_str().unwrap()])
                .output()
                .expect("failed to run read_quality.py");
